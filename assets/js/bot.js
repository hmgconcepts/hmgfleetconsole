...
'🏢 **HMG CONCEPTS** (His Marvellous Grace) — founded by Adewale Samson Adeagbo, Lagos, Nigeria — builds enterprise platforms on free tools: School Connect, Tutoring Connect, DramaConnect Enterprise, HMG Academy CBT Pro, and this Fleet Console (by HMG Technologies). Subsidiaries: HMG Academy · HMG Technologies · HMG Media · HMG Gospel. Full story: About & Ecosystem page. Human help: WhatsApp +234 810 086 6322.' },
    { match:['restored 0','0 project','empty backup','restore not working','nothing restored'], reply:
'📥 **"restored 0 projects" is fixed in V1.8.** Cause: a fresh device auto-backed-up its EMPTY fleet on connect, and that empty file became the newest generation. Now empty fleets are never auto-backed-up, filenames carry the project count (…-p12.json), restore picks the newest NON-EMPTY generation automatically, and Settings → 📚 View backups shows every generation with per-file Restore/Delete. If you still see 0: open 📚 View backups — if EVERY generation shows EMPTY, back up first from the device that has your projects.' },
