...
| `compare.html` — **Compare Projects** | Side-by-side: type, billing+amount, score, uptime %, latency, SLO+budget, heartbeat, key expiry, group, last deploy — A better / B better badges + sparklines |
...
## Billing model (V1.9)
- **💎 One-time / lifetime** vs **🔁 Subscription** per project (field on Projects page). One-time: no renewal date needed — field hides automatically, renewal alerts + calendar skip it, license cell shows lifetime, revenue sums on dashboard + cost ledger. Subscription: renewal date tracked, countdown, alerts, calendar, server verdict via sc_license_status RPC (unfakeable). Existing installs auto-migrate truthfully from feeNote/renewal.
...
- **Login gate** — every page requires sign-in; login page has 👁️ show/hide password toggle (WCAG 3.3.8) + same toggle on every password field in Settings; credentials in ONE file
